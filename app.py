...
from streamlit.components.v1 import declare_component, html as components_html
...
try:
    ...
    from tools.dashboard_sql_agent import query_dashboard_sql, resolve_dashboard_entities
...
st.markdown(
    ...
    <style>
        ...
        /* Remove Streamlit's reserved top gutter so the agent starts at the top. */
        ...
        .st-key-agent_messages {
            ...
            padding: 0.75rem 0.75rem 10.25rem !important;
        ...
        [data-testid="stSidebar"] .stChatMessage ol li {
            padding-left: 0.25rem !important;
            margin-bottom: 0.55rem !important;
        }

        [data-testid="stSidebar"] .stChatMessage ol li::marker {
            color: var(--agent-burgundy) !important;
            font-weight: 800 !important;
            font-size: 1.02em !important;
        }

        [data-testid="stSidebar"] .stChatMessage ul li {
...
def _contextual_prompt(prompt: str) -> str:
    ...
    direct_entities = resolve_dashboard_entities(clean)
...
def _wants_hs_code(question: str) -> bool:
    """Show tariff codes only when the user explicitly asks for or supplies one."""
    q = str(question or "").lower()
    if re.search(r"\b(?:hs|h\.s\.)\s*(?:code|4|6)?\b", q):
        return True
    if re.search(r"\b(?:harmonized|harmonised|tariff|customs|product)\s+code\b", q):
        return True
    # A standalone 4–6 digit non-year token is treated as an explicit code request.
    return any(not looks_like_year(token) for token in re.findall(r"\b\d{4,6}\b", q))


def _hide_hs_codes_unless_requested(text: str, question: str) -> str:
    """Keep product names primary and remove codes from user-facing prose by default."""
    value = str(text or "")
    if _wants_hs_code(question):
        return value
    # Remove dedicated code lines and common inline code decorations.
    value = re.sub(r"(?mi)^\s*[-*]?\s*\*\*HS(?:4|6| Code)?\*\*\s*:\s*`?\d{4,6}`?\s*\n?", "", value)
    value = re.sub(r"\s*\[?`\d{6}`\]?", "", value)
    value = re.sub(r"\s*\(HS(?:4|6)?\s*\d{4,6}\)", "", value, flags=re.IGNORECASE)
    value = re.sub(r"\bHS(?:4|6)?\s*[:#-]?\s*\d{4,6}\b", "", value, flags=re.IGNORECASE)
    value = re.sub(r"\n{3,}", "\n\n", value)
    return value.strip()


def _indicator_requested(question: str, indicator: str) -> bool:
    q = str(question or "").lower()
    aliases = {
        "expy": ("expy", "export sophistication", "sophistication"),
        "hhi": ("hhi", "concentration", "herfindahl"),
        "rca": ("rca", "revealed comparative advantage", "competitiveness"),
        "unrealized potential": ("unrealized potential", "unrealised potential", "export potential", "potential"),
        "performance status": ("performance status", "overperform", "underperform", "performance"),
        "priority": ("priority", "prioritize", "prioritise"),
        "cagr": ("cagr", "compound annual growth", "growth rate"),
    }
    return any(term in q for term in aliases[indicator])


def _filter_unrequested_indicators(text: str, question: str) -> str:
    """Hide specialist indicators from ordinary replies unless the user asks for them."""
    indicator_patterns = {
        "expy": r"\bEXPY\b",
        "hhi": r"\bHHI\b",
        "rca": r"\bRCA\b|revealed comparative advantage",
        "unrealized potential": r"unreali[sz]ed potential|export potential",
        "performance status": r"performance status|overperforming|underperforming",
        "priority": r"\bpriority\b",
        "cagr": r"\bCAGR\b|compound annual growth",
    }
    kept: list[str] = []
    for line in str(text or "").splitlines():
        remove = False
        for indicator, pattern in indicator_patterns.items():
            if not _indicator_requested(question, indicator) and re.search(pattern, line, flags=re.IGNORECASE):
                remove = True
                break
        if not remove:
            kept.append(line)
...
def _present_agent_response(response: Any, question: str) -> str:
    formatted = format_agent_response(response)
    formatted = _hide_hs_codes_unless_requested(formatted, question)
    formatted = _filter_unrequested_indicators(formatted, question)
    return _normalise_markdown(formatted)
...
def _normalise_markdown(text: str) -> str:
    ...
    for raw_line in value.replace("\t", " ").splitlines():
        ...
        elif re.match(r"^\s*(?:\(?\d{1,2}\)|\d{1,2}[-:])\s+", line):
            number = re.search(r"\d{1,2}", line).group(0)
            body = re.sub(r"^\s*(?:\(?\d{1,2}\)|\d{1,2}[-:])\s+", "", line)
            line = f"{number}. {body}"
        lines.append(line)
    value = "\n".join(lines)
    value = re.sub(r"\n{3,}", "\n\n", value).strip()
...
def _aggregate_diagnostic_answer(
    ...
    if destination and not resolved_hs:
        ...
        parts.extend([
            ...
            "",
            "- This diagnoses the market portfolio as a whole. A product-specific diagnosis requires a named product.",
        ])
...
def answer_agent_prompt(prompt: str) -> tuple[str, list[dict]]:
    ...
    if not named_diagnostic:
        ...
        if universal_result.matched and universal_result.confidence >= 0.90:
            ...
            return _present_agent_response(universal_result.answer, prompt), [{
    ...
    if aggregate_diagnostic is not None:
        return (_present_agent_response(aggregate_diagnostic[0], prompt), aggregate_diagnostic[1])
    ...
    if dashboard_result.matched and not (
        ...
        if dashboard_result.confidence < 0.90 and dashboard_result.entities.get("hs6"):
            exact_answer = (
                "**Match note**\n\n"
                "I used the closest product match in the dashboard. Check the product name shown before relying on the result.\n\n"
                + exact_answer
            )
        ...
        return _present_agent_response(exact_answer, prompt), [{
            "source": "dashboard_component/dashboard.html",
    ...
    if not is_product_specific:
        ...
        return _present_agent_response(response_text, prompt), rag_chunks
    ...
    if not destination:
        ...
        if product_profile.matched:
            ...
            return _present_agent_response(product_profile.answer, prompt), [{
                "source": "dashboard_component/dashboard.html",
    ...
    return _present_agent_response(output_data, prompt), []


def _scroll_agent_to_latest() -> None:
    """Keep the newest user question and answer visible above the fixed composer."""
    components_html(
        """
        <script>
        (function () {
          function scrollLatest() {
            try {
              ...
              const box = doc.querySelector('.st-key-agent_messages');
              if (!box) return;
              box.scrollTop = box.scrollHeight;
              const anchor = box.querySelector('#agent-chat-bottom');
              if (anchor) {
                anchor.scrollIntoView({block: 'end', behavior: 'auto'});
              } else {
                const messages = box.querySelectorAll('[data-testid="stChatMessage"]');
          ...
          [40, 140, 320, 700, 1200].forEach(ms => setTimeout(scrollLatest, ms));
        })();
        </script>
        """,
        height=0,
...
def render_agent_sidebar() -> None:
    ...
    with st.container(key="agent_messages"):
        for msg in st.session_state.messages:
            ...
            with st.chat_message(role, avatar=avatar):
                ...
                st.markdown(content)
        st.markdown('<div id="agent-chat-bottom" aria-hidden="true"></div>', unsafe_allow_html=True)

    _scroll_agent_to_latest()
    ...
    try:
        with st.spinner("Analyzing dashboard data…"):
            response_text, _rag_chunks = answer_agent_prompt(clean_prompt)
        final_text = _present_agent_response(response_text, clean_prompt)
